...
test('should print yaml diagnostics if included', function(t) {
	...
	reporter.onRunStart([]);
	reporter.onBrowserStart(mosaic);
	reporter.specFailure(mosaic, {description: 'Failure', time: 1, suite: ['SampleTest'], log: ['Big\nScary\nMultiline\nString\nAnd here\'s some "quotes"!']});
	reporter.onBrowserComplete(mosaic);
	reporter.onRunComplete([]);
	...
	t.equals(reporter.write.getCall(1).args[0], '# Other (Solaris)\n', 'reports browser');
	t.equals(reporter.write.getCall(2).args[0], 'not ok 1 Other (Solaris) :: SampleTest :: Failure\n', 'reports test');
	t.equals(reporter.write.getCall(3).args[0], '  ---', 'prints yaml header');
	t.equals(reporter.write.getCall(4).args[0], '\n    failures: \n      - |\n        Big\n        Scary\n        Multiline\n        String\n        And here\'s some "quotes"!\n', 'prints error information');
